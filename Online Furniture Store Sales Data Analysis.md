...
including the transaction date, product details, payment details, and location data.
...
How many transactions were made by customers in each state?
What are the most popular payment methods?
Who are the frequent customers with more than 3 transactions?
What is the total sales revenue for each country, as a percentage of total sales revenue?
What are the names and payment types of all transactions made by customers in specific states in a specific year?
These queries help in understanding the sales trends, customer behavior, and product popularity, which can be crucial for business 
planning and strategic decisions.

This project is a great example of how SQL can be utilized for data analysis and business intelligence purposes. 
Feel free to clone the repository and run the queries to get a sense of the data and the insights that can be derived from it.
*/
...
SELECT country, product, COUNT(*) as popularity
FROM sales
GROUP BY country, product
ORDER BY popularity DESC;

SELECT product, COUNT(*) as popularity
FROM sales
GROUP BY product
ORDER BY popularity DESC;
...
/*What are the most popular payment types used in the database?*/
SELECT payment_type, COUNT(*) as popularity
FROM sales
GROUP BY payment_type
ORDER BY popularity DESC;

/*What are the names and account creation dates of all customers who have made more than 3 transactions in the database?*/

SELECT name, account_created
FROM sales
GROUP BY name
HAVING COUNT(*) > 1;

/*Calculate the total sales revenue for each country, and display it as a percentage of the total sales revenue for all countries.*/

SELECT 
  country,
  SUM(price) as total_sales,
  (SUM(price) / (SELECT SUM(price) FROM sales)) * 100 as percentage_of_total
FROM sales
GROUP BY country;

/*What are the names and payment_types of all transactions made by customers in California or New Yorkin 2020?*/

SELECT name, payment_type FROM sales
WHERE (state = "New York" OR state ="California") AND (transaction_date = 2020);
